...
def transfer_learning_soft(base_model, num_clases, pipeline):
    ...
    if pipeline['layer_percent'] == 1:
        for layer in base_model.layers[:]:
            layer.trainable = True
    elif pipeline['layer_percent'] > 0 and pipeline['layer_percent'] < 1:  
...
def entrenamiento(kfold,etapa,datos,arquitectura,train_epochs,batch_epochs,early_stopping,iteracion,batch_size,models_info,pipeline):
    ...
    if etapa == 'train' or etapa == 'train_EL':
        ...
        finetune_model = transfer_learning_soft(base_model, clases, pipeline["stage_config"][iteracion])
...
def label_stats(df_EL, df_LC):
    labeling_stats = {}

    if len(df_EL) > 0:
        df_EL_stats = estimate_stats_scores(df_EL)
        df_EL['gth'] = df_EL['patch_name'].apply(lambda x:x.split('/')[-1].split('_')[-1][0])
        df_EL_TP = df_EL[ ( df_EL['grade_'] == df_EL['gth'] ) ]
        ...
        df_LC['gth'] = df_LC['patch_name'].apply(lambda x:x.split('/')[-1].split('_')[-1][0])
        df_LC_TP = df_LC[ ( df_LC['grade_'] == df_LC['gth'] ) ]
    ...
    for kfold in range(1):
        ...
        for iteracion in range(numero_lotes*1):
            ...
            for model in model_zoo:
                ...
                model_memory , model_performance = entrenamiento(kfold,etapa,datos,model,train_epochs,batch_epochs,early_stopping,iteracion,batch_size,models_info,pipeline)            
                #models_info.append([model_memory , model, model_performance['val_acc']])

                # TODO -> TAKE FROM MODELS_INFO TOP MODELS BASE ON VAL_ACC
                models_info[model] = {
                ...
                #models_info.append([model_memory , model, model_performance['test2_acc']])
            ...
            #df_models_performance = pd.DataFrame(models_info, columns=['model_path','model_architecture','model_test2_acc'])
            #top3_models = df_models_performance.sort_values('model_test2_acc',ascending=False).iloc[:3,:].reset_index().drop('index',axis=1)
...
if dataset == 'gleasson':
    ...
    pipeline['id'] = 15
    pipeline["path_label_stats"] = "data_label_stats_15.pickle"
...
#model_zoo = [ 'InpcetionV3', 'InceptionV4', 'ResNet50' ]
