...
def transfer_learning_soft(base_model, num_clases, pipeline):
    ...
    if pipeline['layer_percent'] > 0 and pipeline['layer_percent'] < 1:  
        ...
        print('estoy usando TL suave! ->', len(base_model.layers[:]), split_layer , len(base_model.layers[:split_layer]) )
...
def entrenamiento(kfold,etapa,datos,arquitectura,train_epochs,batch_epochs,early_stopping,iteracion,models_info,pipeline):
    ...
    if etapa == 'train' or etapa == 'train_EL':
        finetune_model = transfer_learning_soft(base_model, pipeline["class_num"], pipeline["stage_config"][iteracion])
...
def labeling(etapa,modelo1,modelo2,modelo3,arquitectura1,arquitectura2,arquitectura3,EL,LC,datos,pipeline,iteracion,models_info):
    ...
    for i in range(len(df1)):
        ...
        if c1 or c2:
            ...
            EL.append([df2['Filename'][i], df2['Predictions'][i], arch_scores])
            EL_iter.append([df2['Filename'][i], df2['Predictions'][i], arch_scores])
            etiquetados_EL += 1
        else:
...
def ssl_global( archivos, model_zoo, csvs, pipeline ):
    ...
    for kfold in range(1):
        ...
        for iteracion in range(numero_lotes*1):
            ...
            #df_EL = pd.DataFrame(EL_iter, columns=[x_col_name, y_col_name, 'arch_scores']) # EXP28
            #df_LC = pd.DataFrame(LC_iter, columns=[x_col_name, y_col_name, 'arch_scores']) # EXP28
            ...
            #df_train_EL = df_EL.iloc[:,:2].copy() # EXP28
            print(df_train)
            print(df_EL)
            print(df_EL.iloc[:,:2])
            print(df_train_EL)
